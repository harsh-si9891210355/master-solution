...
const MATRIX_COLORS: Record<'online' | 'offline' | 'maintenance', string> = {
    online:      COLOR_ONLINE,
...
export const CamerasKpis = ({ data, locale }: { data: CamerasTabData; locale: string }) => {
    ...
    const kpis = [
        { label: t('cameras_tab.kpis.total'),       value: fmt(194), sub: t('cameras_tab.kpis.total_sub'),                                                       accent: COLOR_PRIMARY,     icon: '📹' },
        { label: t('cameras_tab.kpis.online'),       value: fmt(5),   sub: fmtPct(Math.round((5 / 194) * 100)) + ' ' + t('cameras_tab.kpis.online_pct'),          accent: COLOR_ONLINE,      icon: '🟢' },
        {
            label: t('cameras_tab.kpis.offline'),    value: fmt(166), sub: t('cameras_tab.kpis.offline_sub'),
            accent: COLOR_OFFLINE, icon: '🔴', clickable: true,
            onClick: () => navigate('/cameras', { state: { statusFilter: 'inactive' } }),
        },
        { label: t('cameras_tab.kpis.maintenance'), value: fmt(23),  sub: t('cameras_tab.kpis.maintenance_sub'),                                                  accent: COLOR_MAINTENANCE, icon: '🔧' },
    ];
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 20, marginBottom: 20, width: '100%' }}>
            <div className="db-panel db-widget-panel">
                ...
                <ResponsiveContainer width="100%" height={210}>
                    <BarChart data={data.healthTrend} margin={{ top: 5, right: 10, bottom: 0, left: -20 }}>
                        ...
                        <Bar dataKey="online"  name={t('cameras_tab.health_trend.online_label')}  fill={COLOR_ONLINE}  radius={[4,4,0,0]} stackId="a" />
                        <Bar dataKey="offline" name={t('cameras_tab.health_trend.offline_label')} fill={COLOR_OFFLINE} radius={[4,4,0,0]} stackId="a" />
                    </BarChart>
                </ResponsiveContainer>
            </div>
            ...
            <div className="db-panel db-widget-panel" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                ...
                <div style={{
                    background: `linear-gradient(135deg, ${COLOR_PRIMARY}1A 0%, #818CF80D 100%)`,
...
// ─── Camera Health Matrix ────────────────────────────────────────────────────
export const CameraHealthMatrix = ({ data }: { data: CamerasTabData }) => {
    const cameras = Array.from({ length: data.totalCameras ?? 194 }).map((_, i) => {
        ...
        return { id: `CAM-${String(id).padStart(3, '0')}`, status };
    });
    ...
    // Legend order: online first, then offline, then maintenance
    const legendItems: Array<{ key: 'online' | 'offline' | 'maintenance'; label: string }> = [
    ...
    return (
        <div className="db-panel db-widget-panel" style={{ width: '100%', marginBottom: 20 }}>
            <p className="db-panel-title">Camera Health Matrix (Quick View)</p>
            <p className="db-panel-sub" style={{ marginBottom: 16 }}>
                Real-time status grid for all {cameras.length} cameras. Hover for quick details.
            </p>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {cameras.map(cam => (
                    <div
                        key={cam.id}
                        title={`${cam.id} — ${cam.status}`}
                        style={{
                            width: 14, height: 14, borderRadius: 3, flexShrink: 0,
                            background: MATRIX_COLORS[cam.status],
                            boxShadow: `0 0 4px ${MATRIX_COLORS[cam.status]}99`,
                        }}
                    />
                ))}
            </div>

            {/* Legend — each item uses MATRIX_COLORS[key] for both dot and text */}
            <div style={{ display: 'flex', gap: 20, marginTop: 14 }}>
                {legendItems.map(({ key, label }) => (
                    <span
                        ...
                        style={{
                            fontSize: 12,
                            color: MATRIX_COLORS[key],   // ← uses palette, not hardcoded
                            display: 'flex',
                            alignItems: 'center',
                            gap: 5,
                        }}
                    >
                        <span style={{
                            ...
                            background: MATRIX_COLORS[key],   // ← uses palette, not hardcoded
...
export const CamerasHub = ({ data, locale }: { data: CamerasTabData; locale: string }) => {
    ...
    const chartSlice = locationHealthData.slice(chartPage * chartPageSize, (chartPage + 1) * chartPageSize);
    ...
    const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => { setSearch(e.target.value); setPage(0); };
    const handleFilterChange = (tab: 'all' | 'online' | 'offline' | 'maintenance') => { setFilter(tab); setPage(0); };
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.3fr', gap: 32, alignItems: 'start' }}>
                <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-heading)' }}>Device Status by Location</p>
                        {chartPageCount > 1 && (
                    ...
                    <ResponsiveContainer width="100%" height={chartSlice.length * 38 + 40}>
                        <BarChart data={chartSlice} layout="vertical" margin={{ top: 5, right: 10, left: 15, bottom: 5 }}>
                            ...
                            <Bar dataKey="offline"     name="Offline"     stackId="a" fill={COLOR_OFFLINE} radius={[0, 4, 4, 0]} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
                ...
                <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                        ...
                        <div className="db-search-wrapper" style={{ minWidth: 160 }}>
                            ...
                            <input type="text" placeholder="Search Cam ID..." value={search} onChange={handleSearchChange} className="db-search-input" style={{ padding: '4px 8px 4px 22px', fontSize: 11 }} />
                        </div>
                    </div>
                                ...
                                <tr key={cam.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                    ...
                                    <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                                        <span style={{
                                            ...
                                            background: MATRIX_COLORS[cam.status],
                                            boxShadow: `0 0 6px ${MATRIX_COLORS[cam.status]}`
                                        }} />
                                    </td>
                                </tr>
                            ))}
                            {filteredCameras.length === 0 && (
                                <tr><td colSpan={4} style={{ textAlign: 'center', padding: '24px 0', color: 'var(--text-dim)' }}>No devices matched criteria.</td></tr>
                            )}
                        </tbody>
                    </table>
