...
import { cameraService, type RecordingSpan, type StreamConfig } from './api/cameraService';
...
const DVR_WINDOW_MINUTES = DVR_WINDOW_HOURS * 60;

/**
 * IMPORTANT:
 * MediaMTX recording availability always lags behind
 * WebRTC live edge slightly.
 *
 * This safety buffer prevents rewind requests from
 * seeking into non-finalized fragments.
 */
...
const formatTimelineLabel = (timestampMs: number) => {
    const wallTime = new Date(timestampMs);

    return (
        wallTime.toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit',
        })
...
export const DVRPlayer = ({
    ...
    const [spans, setSpans] =
        useState<RecordingSpan[]>([]);

    const [playbackSrc, setPlaybackSrc] =
        useState<string | null>(null);

    const [liveFrameKey, setLiveFrameKey] =
        useState(0);

    const normalizedSpans = useMemo<NormalizedSpan[]>(
        () =>
            spans
                .map((span) => ({
                    ...span,
                    startMs: new Date(span.start).getTime(),
                    endMs: new Date(span.end).getTime(),
                }))
     ...
     * SAFE rewind edge
     *
     * Avoid seeking into non-finalized recording fragments.
     */
    const latestMs =
        rawLatestMs !== null
            ? rawLatestMs - DVR_EDGE_SAFETY_MS
            : null;

    /**
     * Fixed 12h DVR timeline
     */
    const earliestMs =
        latestMs !== null
            ? latestMs - (DVR_WINDOW_HOURS * 60 * 60 * 1000)
            : null;
    ...
    useEffect(() => {
        ...
        setSliderValue(DVR_WINDOW_MINUTES);
    ...
    const setPlaybackStateFromTimestamp = (
        ...
        const lagSec =
            Math.max(
                0,
                (latestMs - timestampMs) / 1000
            );

        setSliderValue(
            (timestampMs - earliestMs) / 60000
        );

        if (
            lagSec <= config.live_edge_threshold_s
        ) {
            setIsAtLiveEdge(true);
            setTimeLabel('LIVE');
            return;
        ...
        setTimeLabel(
            formatTimelineLabel(timestampMs)
        );
    };
    ...
    const updatePlaybackPosition = () => {
        ...
        const currentWallClockMs = Math.min(
            activeClip.startMs +
    ...
    const findSpanForTimestamp = (
        ...
        normalizedSpans.find(
            (item) =>
                targetMs >= item.startMs &&
                targetMs < item.endMs
        )
        ??
        normalizedSpans.find(
            (item) => targetMs < item.startMs
        )
        ??
        normalizedSpans[
    ...
    const seekWithinActiveClip = (
        ...
        if (
            targetMs < activeClip.startMs ||
            targetMs >= activeClip.endMs
        ) {
            return false;
        }
    ...
    const startPlaybackAt = (
        ...
        if (seekWithinActiveClip(targetMs)) {
            return;
        }

        const span =
            findSpanForTimestamp(targetMs);
        ...
        const src =
            ...
            `&duration=${clipDurationS.toFixed(3)}`;
    ...
    const handleSeekTarget = (
        ...
        startPlaybackAt(clampedTargetMs);
    };
    ...
    const fillPct =
        `${(sliderValue / DVR_WINDOW_MINUTES) * 100}%`;

    const visibleEvs = events.filter((ev) => {
